...
async def run(
    ...
    async with connect(
        ...
        if parsed.scheme == "wss":
            ...
            what_do_u_want_to_do = input("Type 1 for registration with RS or 2 for to get a file from Other Server")
            await ws.send(what_do_u_want_to_do)
            mes1 = await ws.recv()
            if len(mes1)>1:
                print(mes1)
            if mes1 == "Sorry I am not a registration Server. Contact RS":
                await ws.close()
            elif mes1 == "Sorry I am not your peer. I cannot send you Files.":
                await ws.close()
            elif what_do_u_want_to_do == "1":
                print("OK. Welcome to registration Server. What do you want to do next")
                while(1):
                    what_next = input("Type 1 for register with RS, 2 to get the list of active peers, 3 to leave.")
                    if what_next == "1":
                        s = socket.socket(socket.AF_INET,socket.SOCK_DGRAM)
                        s.connect(("8.8.8.8",80))
                        ip = (s.getsockname()[0])
                        port = "4433"
                        msg1 = "I want to register myself."+"#"+str(ip)+ "#"+port
                        await ws.send(msg1)
                        mes4 = await ws.recv()
                        print(mes4)
                    elif what_next == "2":
                        msg2 = "Can you please provide me a list of active peers."
                        await ws.send(msg2)
                        mes5 = await ws.recv()
                        list_of_peers = mes5.split("@")
                        count = 0
                        for i in list_of_peers:
                            if i == '[]':
                                count+=1
                        for i in range(count):
                            list_of_peers.remove('[]')
                        set_of_peers = set(list_of_peers)
                        final_list_of_peers = list(set_of_peers)
                        print(final_list_of_peers)
                    elif what_next == "3":
                        s = socket.socket(socket.AF_INET,socket.SOCK_DGRAM)
                        s.connect(("8.8.8.8",80))
                        ip = (s.getsockname()[0])
                        port = "4433"
                        msg3 = "I want to leave."+";"+str(ip)+ ";"+port
                        await ws.send(msg3)  
                        mes6 = await ws.recv()
                        print(mes6)
                        await ws.close()
            ...
            # send some messages and receive reply
                for i in range(1):
                    ...
                    list_of_rcvd_files = message.split(";")
                    #print("< " + message)
                    print(list(set(list_of_rcvd_files)))
                #file_client_needs = "A.txt"
                file_client_needs = input("Enter a file name you want from the server?")
                if file_client_needs in list_of_rcvd_files:
                    print("So, you need :",file_client_needs)
                else:
                    print("I don't have this",file_client_needs,". I can only send you what all I have")
                
                time_in = time.time()
                await ws.send(file_client_needs)
                g = open("examples\\"+file_client_needs,'a')
                message_from_server = await ws.recv()
                list12 = message_from_server.split(";;;")
                no_of_iterations = int(list12[1])
                g.write(list12[0])
                for i in range(no_of_iterations-1):
                    await ws.send("next")
                    message_from_server = await ws.recv()
                    g.write(message_from_server)
                time_out = time.time()
                time_diff = time_out - time_in
                len_of_message = (os.stat("examples\\D.txt").st_size)*8
                if time_diff >0:
                    throughput = len_of_message/time_diff
                    print("time taken for file transfer",time_diff)
